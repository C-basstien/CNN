...
double*** convolve(char*** im, double**** ker,unsigned int* im_size, unsigned int* ker_size, double * bias)
{
	/**Get the dimmension**/
		...
		for(unsigned int canal_index=0;canal_index< im_canal;canal_index++)
		{
			for(unsigned int x=0;x< im_width+ker_width;x++)
			{
				for(unsigned int y=0;y< im_height+ker_height;y++)
				{
					...
					buff[x][y][filter_index]=buff[x][y][filter_index]+bias[filter_index];
				}
			}
		}
