...
def troncature(im):#this function withdraw the border of an image to obtain a 24x24image 
    ...
    return im_tronc
#input: image n'importe taille
#output: tableau meme taille et normalise
...
def maxpool(vect):
    ...
    stride=2
    size=3
    buff=np.zeros((size,size))
    out=np.zeros((n_x/stride,n_y/stride,n_channel))
    for i_channel in range(n_channel):
        for i_x in range(0,n_x-size, stride):
            for i_y in range(0,n_y-size, stride):
